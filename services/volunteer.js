  ...
  try {
    ...
    if (!volunteer)
      return res
        ...
        .json({ message: "Volunteer not found" });
...
exports.applyToHelpRequest = async (req, res) => {
  try {
    ...
    const existingApplication = await VolunteerHelpRequest.findOne({
      where: { volunteerId, helpRequestId },
    });
    if (existingApplication)
      return res
        .status(HTTP_STATUS.BAD_REQUEST)
        .json({ message: "You already applied to this help request" });
...
exports.cancelApplication = async (req, res) => {
  try {
    ...
    return res
      .status(HTTP_STATUS.OK)
      .json({ message: "Application cancelled successfully" });
  } catch (error) {
    handleError(res, error);
  }
};
  ...
  try {
    ...
    if (
      ...
      return res.status(HTTP_STATUS.BAD_REQUEST).json({
        message:
          "Volunteer profile is incomplete for matching (location, skills, or availability missing)",
      });
    }
